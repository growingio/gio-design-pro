...
import { AsyncState, useAsyncFn } from '../useAsync';
...
describe('useAsyncFn', () => {
  ...
  describe('the callback can be awaited and return the value', () => {
    let hook: any;
    const adder: AdderFn = async (a?: number, b?: number): Promise<number> => (a || 0) + (b || 0);
  ...
  it('should only consider last call and discard previous ones', async () => {
    ...
    const delayedFunction1 = () =>
      new Promise<number>((resolve) => queuedPromises.push({ id: 1, resolve: () => resolve(1) }));
    const delayedFunction2 = () =>
      new Promise<number>((resolve) => queuedPromises.push({ id: 2, resolve: () => resolve(2) }));
